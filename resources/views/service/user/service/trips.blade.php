...
<script src="{{ asset('assets/plugins/data-tables/js/dataTables.bootstrap.min.js')}}"></script>
   ...
   $('body').on('click', '.dispute', function(e) {
        ...
        provider_id = $(this).data('provider_id');
         $.ajax({
            type: "GET",
            url: getBaseUrl() + "/user/get_service_request_dispute/"+dispute_id,
   ...
   //    }, 1000);
   $.ajax({
      url:  getBaseUrl() + "/user/getdisputedetails",
      type: "GET",
      beforeSend: function (request) {
               showLoader();
            },
   ...
   $('.validateForm').validate({
		...
		submitHandler: function(form,e) {
            ...
            var url = getBaseUrl() + "/user/service_request_dispute/"+dispute_id;
   ...
   $('body').on('click', '.tripdetails', function(){
         ...
         success:function(data){
            ...
            var item=result.service;
             ...
             var  starvalue=`<span class ="star" style="cursor: default;">`;
               ...
               $('.pickup_time').text(new Date( item.started_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })); 
    ...
    $('body').on('click', '.upcomingdetails', function(){
      ...
      $.ajax({
         type:"GET",
         url: getBaseUrl() + "/user/upcoming/trips/service/"+id,
         headers: {
               Authorization: "Bearer " + getToken("user")
         },
         beforeSend: function (request) {
   ...
   $(document).on('click', '#cancel_req', function(e){
      ...
      upcoming_table = upcoming_table.DataTable( {
        ...
        "lengthChange": false,
        "ajax": {
            "url": getBaseUrl() + "/user/upcoming/trips/service",
            "type": "GET",
            "beforeSend": function (request) {
               showLoader();
            },
