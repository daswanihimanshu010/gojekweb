      ...
      <div class="col-md-12 p-0 add-money-section">
         <div class="col-md-6 col-xl-3 col-xs-12 top small-box green">
               <div class="left">
                  ...
                  <span class="currency"></span> <h1 class="wallet_balance"></h1>
               </div>
            </div>
            @if($card==1)
...
@parent
   ...
   payment_table = payment_table.DataTable({
        ...
        "columns": [
            ...
            { "data": "amount" ,render: function (data, type, row, meta) {
               return  row.provider.currency_symbol + parseFloat(data).toFixed(2);
            } },
   ...
   $('.validateCardForm').validate({
		...
		messages: {
         card_id: { required: "Card is required." },
		...
		submitHandler: function(form) {
         ...
         var url = getBaseUrl() + "/provider/add/money";
         saveRow( url, null, data, "provider",'/provider/wallet');
        // location.reload();
         // var url = getBaseUrl() + "/provider/add/money";
         // $.ajax({
         //    type:'POST',
