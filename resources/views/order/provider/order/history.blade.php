  ...
  //For get the dispute name
   ...
   $('body').on('click', '.dispute', function(e) {
        ...
        $("#DisputeOrder").val(dispute_id);
         $.ajax({
            type: "GET",
            url: getBaseUrl() + "/provider/history-dispute/order/"+dispute_id,
            beforeSend: function (request) {
               showLoader();
            },
            headers: {
   ...
   $('.validateForm').validate({
      ...
      trips_table = trips_table.DataTable( {
        ...
        "columns": [
         ...
         { "data": "assigned_at" ,render: function (data, type, row) {
      ...
      $.ajax({
         ...
         success:function(data){
            ...
            ongoingInitialize(result);
               ...
               $('.orderDate').text(new Date( result.created_at ).toLocaleDateString("en-Us")); 
               ...
               var grossAmount = result.order_invoice.gross.toFixed(2);
