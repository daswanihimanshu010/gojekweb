...
class StartedComponent extends React.Component {
    ...
    handleSendMessage = e => {
        if((e.which == 13 || e.which === undefined) && $('input[name=message]').val() != '') {
            ...
            chatSocket.emit('send_message', {room: `room_${window.room}_R${request_id}_U${user_id}_P${provider_id}_${admin_service_id}`, url: getBaseUrl() + "/chat", salt_key: window.salt_key, id: request_id, admin_service: admin_service_id, type: 'provider', user: user, provider: provider, message:  message.val() });
            ...
            <div className="row">
                <div  className={this.props.request.is_drop_location == 1 ? "service-card col-xs-12 col-sm-12 col-md-12 col-lg-6 available m-0" : "service-card col-xs-12 col-sm-12 col-md-12 available m-0" } >
                    <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-0">
                        ...
                        {(this.props.request.status == "ARRIVED" && getSiteSettings().transport.ride_otp=="1") ? <a id="arrived" className="btn btn-primary btn-md mr-2"  data-toggle="modal" onClick={this.handleOtpModal} >Pick <i className="fa fa-check" aria-hidden="true"></i></a> : ""}
                        {(this.props.request.status == "ARRIVED" && getSiteSettings().transport.ride_otp=="0") ? <a id="arrived" className="btn btn-primary btn-md mr-2"   onClick={this.handleUpdateStatus.bind(this, 'PICKEDUP')} >Pick & Start <i className="fa fa-check" aria-hidden="true"></i></a> : ""}  
...
class InvoiceComponent extends React.Component {
    ...
    handleRating = e => {
        ...
        let admin_service_id = this.props.provider.service.admin_service_id;
