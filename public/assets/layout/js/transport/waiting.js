class MainComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        socket.on('socketStatus', function (data) {
            console.log('You are connected to private room');
        });
...
class AcceptedComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        var admin_service_id = 'TRANSPORT';
    	...
    	chatSocket.emit('joinPrivateChatRoom', `room_${window.room}_R${id}_U${user_id}_P${provider_id}_${admin_service_id}`);
    ...
    handleSendMessage = e => {
        if((e.which == 13 || e.which === undefined) && $('input[name=message]').val() != '') {
            ...
            chatSocket.emit('send_message', {room: `room_${window.room}_R${request_id}_U${user_id}_P${provider_id}_${admin_service_id}`, url: getBaseUrl() + "/chat", salt_key: window.salt_key, id: request_id, admin_service: admin_service_id, user: user, provider: provider, type: 'user',message:  message.val() });
...
class InvoiceComponent extends React.Component {
    ...
    handleRating = e => {
        ...
        let admin_service_id = this.props.request.service_type.admin_service_id;
        ...
        $.ajax({
            ...
            data: {
                id, rating, comment, admin_service_id
            },
            success: (data, textStatus, jqXHR) => {
                $("#rating").modal('hide');
                $(".ride-section").removeClass('d-none');
            ...
            <div id="invoice-section" className="tab-pane active dis-column">
                ...
                <div className="col-md-12 col-sm-12 col-lg-12 p-0">
                    <form id="ride_creation">
                        <dl className="dl-horizontal left-right mt-5">
                            ...
                            {this.props.request.distance ? (
                                <span>
                                    ...
                                    <dd>{this.props.request.distance} Kms</dd>
                                </span>
                            ) : ( "" )}
                            ...
                            {this.props.request.payment.toll ? (
                                <span>
                                    <dt>Toll Charge</dt>
                                    <dd>{this.props.request.currency}{this.props.request.payment.toll}</dd>
                                </span>
                            ) : ( "" )}
                        ...
                        {this.props.request.payment_mode != "CASH" ? this.props.request.paid != 1 ? ( <a id="confirm-invoice" onClick={this.handlePayment} data-toggle="modal" data-target="#rating" className="btn btn-secondary btn-block" > Pay Now <i className="fa fa-check" aria-hidden="true" /> </a> ) : ( <a data-toggle="modal" data-target="#rating" className="btn btn-secondary btn-block" > Rate Now <i className="fa fa-check" aria-hidden="true" /> </a> ) : ''}
                    </form>
                </div>
                {/* <!-- Rating Modal --> */}
