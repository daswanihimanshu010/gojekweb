                        ...
                        <div className="col-md-4 p-0 bg-body">
                            ...
                            requests.map((request) =>
                                <div key={request.id} onClick={ this.handleClick.bind(this, request) } className="bg-white m-lr-20 shadow-sm">
                                  {(() => {
                                   switch(request.service.admin_service_name) {
                                        case 'TRANSPORT':
                                        return <div> 
                                            ...
                                            <div className="p-2">
                                                ...
                                                <p className="font_16"> {(request.request.request_type == "MANUAL") ? "Manual" : "Auto" } Assignment : {new Date( request.request.assigned_at ).toLocaleDateString("en-Us")}{" "} {new Date( request.request.assigned_at ).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                                       ...
                                       case 'ORDER':
                                       return <div> 
                                           <div className="ribbon">{request.service.admin_service_name} - {request.request.invoice.items[0].store.store_name}</div>
                                            <a className="btn btn-sm btn-green float-right m-2">{request.request.status}</a>
                                            <div className="p-2">
                                                <p className="font_16 txt_clr_2 fnt_weight_500">{request.user.first_name} {request.user.last_name}</p>
...
class DispatcherRequestComponent extends React.Component {
    ...
    getService = e => {
        ...
        if(e.currentTarget.value != "") {
            ...
            $.ajax({
                url: getBaseUrl() + "/admin/services/list/"+e.currentTarget.value,
                type: "get",
                headers: {
                    Authorization: "Bearer " + getToken("admin")
                },
                                ...
                                <div className="form-group">
                                    ...
                                    <select name="admin_service" className="form-control" id="admin_service" onChange={this.getService}>
                                        ...
                                        {this.state.main_service.map((service, index) => {
                                            return <option key={`c_${index}`} data-service={service.admin_service_name} value={service.id}>{service.display_name}</option>;
                                        })}
                                    </select>
                                </div>
...
class DispatcherAssignComponent extends React.Component {
    ...
    componentDidMount() {
        if(this.props.request.service.admin_service_name=="ORDER"){
        ...
        } else if(this.props.request.service.admin_service_name=="TRANSPORT"){
        ...
        return (
                    <div className="tab-contents row">
                        <div className="col-md-4 p-0 bg-body">
                            ...
                            requests.map((request) =>
                                <div key={request.id} onClick={ this.handleClick.bind(this, request) } className="bg-white m-lr-20 shadow-sm">
                                    <div className="ribbon">{request.service.display_name} - {request.request.service ? request.request.service.service_category.service_category_name : request.request.ride_type.ride_name}</div>
                                    <a className="btn btn-sm btn-green float-right m-2">{request.request.status}</a>
                                    <div className="p-2">
                                        <p className="font_16 txt_clr_2 fnt_weight_500">{request.user.first_name} {request.user.last_name}</p>
                                        <p className="font_14">From: {request.request.s_address}</p>
